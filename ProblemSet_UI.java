...
public class ProblemSet_UI {
		...
		static double [] FET_Part = new double [Déclaration.swarm_size_];
		...
		//	double[][] TC_VM = new double[n][m]; 
			double[][] ET = new double [n][Déclaration.nb_c+Déclaration.nb_f]; 	
			...
			for (int i=0 ; i<n ; i++){	
				...
				if(location.getLoc()[i]>= (Déclaration.nb_c+Déclaration.nb_f)|| location.getLoc()[i]<0 || TET[i][location.getLoc()[i]]<=0)
				{
				    //générer pour cette position (i) une valeur entre 0 et m-1 (comme dans l'initialisation) 
					int xx = (int)(Math.random() * (Déclaration.nb_c+Déclaration.nb_f) );
					while (TET[i][xx]<=0)
						 xx = (int)(Math.random() * (Déclaration.nb_c+Déclaration.nb_f) );
