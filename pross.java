...
public class pross  {
	...
	private double[] pBest = new double[Déclaration.swarm_size_];
	private Vector<Location> pBestLocation = new Vector<Location>();
	private double gBest;
	private Location gBestLocation;
	private int gBestLoc;
	private Location location;
	private double[] fitnessValueList = new double[Déclaration.swarm_size_];//tableau contenant les valeurs de fonct objectif pour chaque particule
	static double [] FET_Part = new double [Déclaration.swarm_size_];//Temps de fin d'execution des particules
...
;
	...
	public void execute(double [][] TET,int n,double[][] DT , double [][] ET,double[] UC_VM, double D) {    //ajouter TET comme param d'entrée pour pouvoir l'ajouter dans evaluate/updateFitnessList
		...
		for(int i=0; i<Déclaration.swarm_size_; i++) { 
		...
		while(t < Déclaration.MAX_ITERATION_ && err > ProblemSet_UI.ERR_TOLERANCE) {
...
//--------------------------------------------------step 2-1 : Update gBest--------------------------------------------------//	
		...
		//	w = W_UPPERBOUND - (((double) t) / MAX_ITERATION) * (W_UPPERBOUND - W_LOWERBOUND);
			System.out.println("...................w===== " + Déclaration.w_);	
...
//--------------------------------------------------step 3 : Update Velocity--------------------------------------------------//	
				...
				newVel[j] = (Déclaration.w_ * p.getVelocity().getPos()[j]) + 
							(r1 * Déclaration.C1_) * (pBestLocation.get(i).getLoc()[j] - p.getLocation().getLoc()[j]) +
							(r2 * Déclaration.C2_) * (gBestLocation.getLoc()[j] - p.getLocation().getLoc()[j]);
...
//--------------------------------------------------step 4 : Update Location--------------------------------------------------//	
			...
			if (t < Déclaration.MAX_ITERATION_){ //condition pour ne pas dépasser le nb itérations demandés
		    System.out.println("-----------The new Localities :-----------  ");
			updateFitnessList(TET,n,DT,UC_VM,D);// il fait update de la fc fitness que si il respecte le nb_itération càd t < MAX_ITERATION
		...
		if(gBestLocation.getLoc()[k2]<Déclaration.nb_c)
		{
			 ressource_name= Déclaration.list_1.get(gBestLocation.getLoc()[k2]).getName();
		}
		else 
			 ressource_name = Déclaration.list_2.get(gBestLocation.getLoc()[k2] - Déclaration.nb_c).getName();
...
//--------------------------------------------------step 5 : Méthode d'initialiser Swarm--------------------------------------------------//	
	...
	public void initializeSwarm(int n,double [][] TET) {
		...
		for(int i=0; i<Déclaration.swarm_size_; i++) { //SWARM_SIZE=taille du swarm=nbre de particule
			p = new Particule_UI();
			...
			for (int j=0; j<n ; j++)
			{
				 x=(int)(Math.random() * (Déclaration.nb_c+Déclaration.nb_f) );
			...
			System.out.println("**********Localité de T["+j+"] :************** "+ loc[j] );
...
//--------------------------------------------------step 6 : Méthode pour changer Fonction objective (liste de fitness value)--------------------------------------------------//	
	...
	public void updateFitnessList(double [][] TET,int n, double[][] DT,double[] UC_VM, double D) {  //ajouter TET comme param entrée
